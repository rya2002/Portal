...
export default function BibliotecaView() {
  ...
  // calcula total de itens conforme a sub-aba ativa
  const totalItens = useMemo(() => {
    return dados.reduce((total: number, semestre: SemestreData) => {
      return total + (activeSubTab === 'artigo' ? semestre.artigos.length : semestre.revistas.length);
    }, 0);
  }, [dados, activeSubTab]);

  // aplica filtro visual por sub-aba ao enviar para o SemestreSection
  const dadosFiltrados = useMemo(() => {
    return dados.map(({ semestre, artigos, revistas }) => ({
      semestre,
      artigos: activeSubTab === 'artigo' ? artigos : [],
      revistas: activeSubTab === 'revista' ? revistas : [],
    }));
  }, [dados, activeSubTab]);
    ...
    <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {dadosFiltrados.length > 0 ? (
          <div>
            {dadosFiltrados.map(({ semestre, artigos, revistas }) => (
              <SemestreSection
                key={semestre}
                semestre={semestre}
                artigos={artigos}
